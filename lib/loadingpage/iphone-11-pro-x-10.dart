...
class updateText10 extends StatefulWidget {
  Scene10 createState() => Scene10();
}
class Scene10 extends State {
  DatabaseReference ref = FirebaseDatabase.instance.ref("Events/Theatre");
  String s1 = "";
  String s2 = "";
  Scene10() {
    ref.child("Befikr/description").once().then((DatabaseEvent event) {
      setState(() {
        s1 = event.snapshot.value.toString().split(r"\n").join("\n");
      });
    });
    ref.child("Hasratein/description").once().then((DatabaseEvent event) {
      setState(() {
